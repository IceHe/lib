...
getLatestPipeline () {
    ...
    local pipeline=`curl --silent \
        --location "https://infra-api.zhenguanyu.com/infra-phoenix-console/api/pipeline/conan-commerce-shipment?branchName=${branchName}&page=0&authorName=${authorName}" \
        --request GET \
        --header "${cookie}" \
        | jq '.list[0]'`
    echo "$pipeline"
...
getPipelineDetail () {
    ...
    local pipelineDetail=`curl --silent \
        --location "https://infra-api.zhenguanyu.com/infra-phoenix-console/api/pipeline/detail/v2/${pipelineId}/${projectIdentity}" \
        --request GET \
        --header "${cookie}"`
    echo "$pipelineDetail"
}
...
triggerBuild () {
    ...
    echo -e "triggerBuild () { … }"
    curl \
        --location "https://infra-api.zhenguanyu.com/infra-phoenix-console/api/action/triggerBuild?projectIdentity=${projectIdentity}&branch=${branchName}" \
        --request POST \
        --header "${cookie}"
    echo
}
...
triggerDeploy () {
    ...
    curl \
        --location "https://infra-api.zhenguanyu.com/infra-phoenix-console/api/action/triggerDeployment?projectIdentity=${projectIdentity}&pipelineId=${pipelineId}&stage=${stage}" \
        --request POST \
        --header "${cookie}"
    echo
}
...
for i in `seq 1 ${times}`; do
    ...
    if [ "$buildStageStatus" == "\"FAILED\"" ]; then
        notify "Pineline ${pipelineId}" "Failed to build branch ${branchName}" \
            "https://console.zhenguanyu.com/#/management/projects/${projectIdentity}/pipelines/${pipelineId}?stage=BUILD"
        exit
    fi
