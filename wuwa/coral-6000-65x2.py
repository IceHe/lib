    ...
    for i in range(loop_count):
        ...
        while not simulator.done():
            ...
            if result == 2:
                ...
                if weapon_count >= 5 * 2:  # 抽满 2 个限定角色的满阶专武
                    break

        if DEBUG:
            print("「抽限定角色池，直到攒够6千大珊瑚」")
        ...
        while not simulator.done():
            ...
            if result == 2:
                ...
                prev_coral = simulator.afterglow_coral
                if resonator_count == 7:
                    print(f"第 {i} 次模拟抽出第 7 个限定五星角色，total convene: {simulator.convene_all}")
                    print(f"切换到版本下半卡池继续抽取另一位限定角色")
                    simulator.counts['限定五星角色'] -= 7
                    # 抽完上半卡池限定五星角色的满6链
                    # 等到下半卡池的限定五星角色继续重新抽
                elif resonator_count == 14:
                    print(f"第 {i} 次模拟抽出第 14 个限定五星角色，total convene: {simulator.convene_all}")
                    print(f"已经抽完一个版本的两个限定角色，继续抽攒大珊瑚")
            elif result == 1:
