...
![grall-vm-intro.png](../.gitbook/assets/grall-vm-intro.png)
...
* OpenJDK Mercurial Repositories : [https://hg.openjdk.java.net/jdk](https://hg.openjdk.java.net/jdk)
...
![clion-debug-config.png](../.gitbook/assets/clion-debug-config.png)
...
![jvm-runtime-data-area.png](../.gitbook/assets/jvm-runtime-data-area.png)
...
![reachability-analysis.png](../.gitbook/assets/reachability-analysis.png)
...
![object-disappear.png](../.gitbook/assets/object-disappear.png)
...
![brooks-pointer-1.png](../.gitbook/assets/brooks-pointer-1.png)

![brooks-pointer-2.png](../.gitbook/assets/brooks-pointer-2.png)
...
![zgc-memory-layout.png](../.gitbook/assets/zgc-memory-layout.png)
...
![multi-mapping.png](../.gitbook/assets/multi-mapping.png)
...
* **Concurrent Relocate** _\( 并发重分配 \)_
  ...
  * **要把 Relocation Set 中的存活的 Object 复制到新的 Region 上**
    * **并为 Relocation Set 中的每个 Region 维护一个 Forward Table** _**\( 转发表 \)**_ **, 记录从旧 Object 到新 Object 的转向关系**
...
![zgc-running.png](../.gitbook/assets/zgc-running.png)
...
![zgc-throughput-test.png](../.gitbook/assets/zgc-throughput-test.png)

![zgc-pause-time-test.png](../.gitbook/assets/zgc-pause-time-test.png)
...
| -XX:+GCLogFileSize-, NumberOfGCLogFiles, UseGCLogFileRotation | Xlog:gc\*:file=\::filecount=\,filesize=\ |
...
| 参数 | 描述 |
| :--- | :--- |
| UseSerialGC | 使用 **Serial + Serial Old** 的收集器组合进行内存回收, **VM 运行在 Client 模式下的默认值** |
| _UseParNewGC_ | _使用 ParNew + Serial Old 的收集器组合进行内存回收,_ _在 JDK 9 后不再支持_ |
| UseConcMarkSweepGC | 使用 **ParNew + CMS + Serial Old** 的收集器组合进行内存回收, Serial Old 收集器将作为 CMS 收集器出现 "Concurrent Mode Failure" 失败后的后备收集器使用 |
| UseParallelGC | 使用 **Parallel Scavenge + Serial Old \( PS MarkSweep \)** 的收集器组合进行内存回收, **JDK 9 之前 VM 运行在 Server 模式下的默认值** |
| UseParallelOldGC | 使用 **Parallel Scavenge + Parallel Old** 的收集器组合进行内存回收 |
| SurvivorRatio | **新生代中 Eden 区域与 Suvivor 区域的容量比值**, **默认为 8** , 代表 Eden : Survivor = 8 : 1 |
...
| HandlePromotionFailure | **是否允许分配担保失败**, 即老年代的剩余空间不足以应付新生代的 整个 Eden 和 Survivor 区的所有对象都存活的极端情况 |
| ParallelGCThreads | _设置并行 GC 时进行内存回收的线程数_ |
| GCTimeRatio | _GC 时间占总时间的比率, 默认值为 99, 即允许 1% 的 GC 时间._ _仅在使用 Parallel Scavenge 收集器时生效_ |
| MaxGCPauseMillis | _设置 GC 的最大停顿时间._ _仅在使用 Parallel Scavenge 收集器时生效_ |
| CMSInitiatingOccupancyFraction | _**设置 CMS 收集器在老年代空间被使用多少后触发垃圾收集**._ _默认值为 68% , 仅在使用 CMS 收集器时生效_ |
| UseCMSCompactAtFullCollection | _设置 CMS 收集器在完成垃圾收集后是否要进行一次内存碎片整理._ _仅在使用 CMS 收集器时生效, 此参数从 JDK 9 开始废弃_ |
| CMSFullGCsBeforeCompaction | _设置 CMS 收集器在进行若干次垃圾收集后再启动一次内存碎片整理._ _仅在使用CMS收集器时生效, 此参数从JDK 9开始废弃_ |
...
| **InitiatingHeapOccupancyPercent** | **设置触发标记周期的 Java 堆占用率阈值. 默认值是 45%**. 这里的 java 堆占比指 non\_young\_capacity\_bytes, 包括 old+humongous |
| UseShenandoahGC | **使用 Shenandoah 收集器**. 该选项在 OracleJDK 中不被支持, 只能在 OpenJDK 12 _或者某些支持 Shenandoah 的 Backport 发行版本使用._ _目前仍然要配合 `-XX:+UnlockExperimentalVMOptions` 使用_ |
...
| **UseNUMA** | **启用 NUMA 内存分配支持**, 目前只有 Parallel 和 ZGC 支持, _以后 G1 收集器可能也会支持该选项_ |
...
[File : NewGenerationMinorGC.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/NewGenerationMinorGC.java)

_Output_

[File : NewGenerationMinorGC.out](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/NewGenerationMinorGC.out)
...
[File : New2OldGeneration.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/New2OldGeneration.java)

_Output_

* `-XX:MaxTenuringThreshold=1`

[File : New2OldGenerationAtAge1.out](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/New2OldGenerationAtAge1.out)

* `-XX:MaxTenuringThreshold=15`

[File : New2OldGenerationAtAge15.out](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/New2OldGenerationAtAge15.out)
...
[File : New2OldGenDynamicStd.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/New2OldGenDynamicStd.java)

_Output_

[File : New2OldGenDynamicStd.out](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/New2OldGenDynamicStd.out)

Sample 2

[File : New2OldGenDynamicStd2.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/New2OldGenDynamicStd2.java)

_Output_

[File : New2OldGenDynamicStd2.out](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/New2OldGenDynamicStd2.out)
...
[File : OldGenHandlePromotionFailure.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/OldGenHandlePromotionFailure.java)

_Output_

[File : OldGenHandlePromotionFailureOff.out](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/OldGenHandlePromotionFailureOff.out)
...
| **jstatd** | JVM Statistics Monitoring Tool Daemon , jstat 的守护程序, 局动一个 RMI 服务器应用程序, 用于监视测试的 HotSpot 虚拟机的创建和终止, 并提供一个界面, 允许远程监控工具附加到在本地系统上运行的虚拟机. 在 JDK 9 中集成到了 JHSDB 中 |
...
| **jconsole** | Java Console, 用于监控 Java 虚拟机的使用 JMX 规范的图形工具. 它可以监控本地和远程和 JVM, 还可以监控和管理应用程序 |
...
| jvisualvm | Java VisualVM, 一种图形化工具, 可在 JVM 中运行时提供有关基于 Java 技术的应用程序 \( Java 应用程序 \) 的详细信息. Java VisualVM 提供内存和 CPU 分析、堆转储分析、内存泄漏检测、MBean 访问和垃圾收集. 从JDK 6 Update 7 开始提供; 从JDK 9 开始不再打包入 JDK 中, 但仍保持更新发展, 可以独立下载 |
...
| 名称 | 主要作用 |
| :--- | :--- |
| _keytool_ | 管理密钥库和证书. 主要用于获取或缓存 Kerberos 协议的票据授权票据. 允许用户查看本地任据缓存和密钥表中的条目 \( 用于 Kerberos 协议 \) |
| _jarsigner_ | 生成并验证 JAR 签名 |
| _policytool_ | 管理策略文件的 GUI 工具, 用于管理用户策略文件 \( .java.policy \) 在 JDK 10 中被移除 |
...
| 名称 | 主要作用 |
| :--- | :--- |
| _native2ascii_ | 本地编码到 ASCII 编码的转换器 \( Native-to-ASCII Converter \) , 用于 "任意受支持的字符编码"和与之对应的 "ASCII 编码和 Unicode 转义" 之间的相互转换 |
...
| _idlj_ | IDL 转 Java 编译器 \( IDL-to-Java Compiler \) , 生成映射 OMG IDL 接口的 Java 源文件, 并启用以 Java 编程语言编写的使用 CORBA 功能的应用程序的 Java 源文件. IDL 意即接口定义语言 \( Interface Definition Language \) |
| _orbd_ | 对象请求代理守护进程 \( Object Request Broker Daemon \) , 提供从客户端查找和调用 CORBA 环境服务端上的持久化对象的功能. 使用 ORBD 代替瞬态命名服务 mameserv. ORBD 包括瞬态命名服务和持久命名服务. ORBD 工具集成了服务器管理器、互操作命名服务和引导名称服务器的功能. 当客户端想进行服务器时定位、注册和激活功能时, 可以与 servertool 一起使用 |
...
| _pack200_ | 使用 Java GZIP 压缩器将 JAR 文件转换为压缩的 Pack200 文件. 压缩的压缩文件是高度压缩下的 JAR, 可以直接部署, 节省带宽并减少下载时间 |
...
| _jjs_ | 对 Nashorn 引擎的调用入口. Nashorn 是基于 Java 实现的一个轻量级高性能 JavaScript 运行环境 |
| _jrunscript_ | Java 命令行脚本外壳工具 \( Command Line Script Shell \) , 主要用于解释执行 JavaScript、Groovy、Ruby 等脚本语言 |
...
* _其中, JConsole 是最古老, 早在 JDK 5 时期就已经存在的虚拟机监控工具_
  * Java SE 11 Docs - Tools References - `jconsole` : [https://docs.oracle.com/en/java/javase/11/tools/jconsole.html](https://docs.oracle.com/en/java/javase/11/tools/jconsole.html)
...
| :--- | :--- | :--- |
| jps -lm | jcmd | N/A |
| jmap -dump \ | jcmd \ GC.heap\_dump | jhsdb jmap --binaryheap |
| jmap -histo \ | jcmd \ GC.class\_histogram | jhsdb jmap --histo |
| jstack \ | jcmd \ Thread.print | jhsdb jstack --locks |
| jinfo -sysprops \ | jcmd \ VM.system\_properties | jhsdb info --sysprops |
| jinfo -flags \ | jcmd \ VM.flags | jhsdb jinfo --flags |
...
[File : JhsdbTestCase.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/JhsdbTestCase.java)
...
[File : JConsoleMonitoringMemory.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/JConsoleMonitoringMemory.java)
...
![jconsole-monitoring-overview.png](../.gitbook/assets/jconsole-monitoring-overview.png)

![jconsole-monitoring-memory-eden-space.png](../.gitbook/assets/jconsole-monitoring-memory-eden-space.png)

![jconsole-monitoring-memory-tenured-gen.png](../.gitbook/assets/jconsole-monitoring-memory-tenured-gen.png)
...
[File : JConsoleMonitoringThreadWait.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/JConsoleMonitoringThreadWait.java)
...
![jconsole-monitoring-thread-wait-before-input-1st.png](../.gitbook/assets/jconsole-monitoring-thread-wait-before-input-1st.png)
...
![jconsole-monitoring-thread-wait-before-input-2nd.png](../.gitbook/assets/jconsole-monitoring-thread-wait-before-input-2nd.png)
...
![jconsole-monitoring-thread-wait-after-input-2nd.png](../.gitbook/assets/jconsole-monitoring-thread-wait-after-input-2nd.png)

**线程死锁**

一个无法再被激活的死锁等待

[File : JConsoleMonitoringDeadLock.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/JConsoleMonitoringDeadLock.java)
...
![jconsole-monitoring-dead-lock-threads.png](../.gitbook/assets/jconsole-monitoring-dead-lock-threads.png)

* 出现线程死锁之后, 点击 JConsole "Threads" 面板的 "Detect DeadLock" 按钮, 将出现一个新的 "Deadlock" Tab

![jconsole-monitoring-dead-lock-detect-deadlock.png](../.gitbook/assets/jconsole-monitoring-dead-lock-detect-deadlock.png)
...
![jvisualvm-startup-n-plugins.png](../.gitbook/assets/jvisualvm-startup-n-plugins.png)

> 双击 `VisualVM` 后

* Overview

![jvisualvm-overview.png](../.gitbook/assets/jvisualvm-overview.png)

* Monitor

![jvisualvm-monitor.png](../.gitbook/assets/jvisualvm-monitor.png)

* Threads

![jvisualvm-threads.png](../.gitbook/assets/jvisualvm-threads.png)

* Sampler - CPU

![jvisualvm-sampler-cpu.png](../.gitbook/assets/jvisualvm-sampler-cpu.png)

* Sampler - Memory

![jvisualvm-sampler-memory.png](../.gitbook/assets/jvisualvm-sampler-memory.png)

* Visual GC

![jvisualvm-visual-gc.png](../.gitbook/assets/jvisualvm-visual-gc.png)

* Tracer

![jvisualvm-tracer.png](../.gitbook/assets/jvisualvm-tracer.png)
...
[File : BTraceTest.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/BTraceTest.java)

* BTraceTestTracingScript.java

[File : BTraceTestTracingScript.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/BTraceTestTracingScript.java)
...
![jvisualvm-tracing-application.png](../.gitbook/assets/jvisualvm-tracing-application.png)
...
* 或者下载独立安装的 JMC 后启动
  * JDK Mission Control \(JMC\) 7  Downloads : [https://www.oracle.com/java/technologies/javase/products-jmc7-downloads.html](https://www.oracle.com/java/technologies/javase/products-jmc7-downloads.html)

JMC 7 的 GUI

* MBean Server

![jmc-mbean-server.png](../.gitbook/assets/jmc-mbean-server.png)
...
![jmc-connect-remote-jvm.png](../.gitbook/assets/jmc-connect-remote-jvm.png)
...
![jmc-start-flight-recording.png](../.gitbook/assets/jmc-start-flight-recording.png)
...
* _可以根据自己的操作系统和处理器型号, 从网上直接搜索、下载编译好的插件, 直接放到 JDK\_HOM E/jre/bin/server 目录 \( JDK 9 以下 \) 或 JDK\_HOME/lib/amd64/server \( JDK 9 或以上 \) 中即可使用_
...
[File : HsdisTest.java](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/HsdisTest.java)

_Prepare_

* _安装 HSDIS 插件 \( on macOS \)_
  * [https://github.com/liuzhengyang/hsdis\#usage](https://github.com/liuzhengyang/hsdis#usage)
...
* Options
  ...
  * `-XX:+PrintAssembly`
    * 输出反汇编内容
    * Reference : [https://wiki.openjdk.java.net/display/HotSpot/PrintAssembly](https://wiki.openjdk.java.net/display/HotSpot/PrintAssembly)
...
[File : HsdisTest.out](https://github.com/IceHe/lib/tree/4e6b7c73229e0e23ff9d6acf7f2ba61d9dacec30/books/src/understand-jvm/HsdisTest.out)
...
* 一个 15万 PV/日 左右的 **在线文档类型网站** 最近更换了硬件系统
  ...
  * 操作系统为 64 位 CentOS 5.4, Resin 作为 Web 服务器
    * Resin : [https://baike.baidu.com/item/resin/8920052](https://baike.baidu.com/item/resin/8920052)
...
* _当然, 第二种部署方案也不是没有缺点的,_ 如果使用逻辑集群的方式来部署程序, 可能会遇到下面这些问题 :
  ...
  * **如果使用 32 位 JVM 作为集群节点的话,** _**各个节点仍然不可避免地**_ **受到 32 位的内存限制**
...
![jboss-cache-protocol-stack.png](../.gitbook/assets/jboss-cache-protocol-stack.png)
...
* **WebLogic** \( Application Server \)
  ...
  * References
    * Oracle WebLogic Server - Wikipedia : [https://en.wikipedia.org/wiki/Oracle\_WebLogic\_Server](https://en.wikipedia.org/wiki/Oracle_WebLogic_Server)
    * WebLogic - 百度百科 : [https://baike.baidu.com/item/weblogic/451978](https://baike.baidu.com/item/weblogic/451978)
* **亲和式集群**
  ...
  * References
    * Web 服务器集群 \( 亲和式集群, 集群中保持用户状态 \) : [https://blog.csdn.net/liao0801\_123/article/details/85042090](https://blog.csdn.net/liao0801_123/article/details/85042090)
* **JBoss Cache**
  ...
  * References
    * JBoss Cache : [https://jbosscache.jboss.org](https://jbosscache.jboss.org)
...
* **Reverse Ajax** \( 反向 AJAX, 也称为 Comet / Server Side Push \)
  ...
  * References
    * Comet \(programming\) - Wikipedia : [https://en.wikipedia.org/wiki/Comet\_\(programming](https://en.wikipedia.org/wiki/Comet_%28programming)\)
    * _反向 Ajax, 第 1 部分: Comet 简介 :_ [https://www.ibm.com/developerworks/cn/web/wa-reverseajax1/index.html](https://www.ibm.com/developerworks/cn/web/wa-reverseajax1/index.html)
* **CometD**
  ...
  * References
    * CometD – Highly Scalable Clustered Web Messaging : [https://cometd.org/](https://cometd.org/)
* Eclipse **Jetty**
  ...
  * References
    * Jetty - Servlet Engine and Http Server : [https://www.eclipse.org/jetty/about.html](https://www.eclipse.org/jetty/about.html)
    * Jetty \(web server\) - Wikipedia : [https://en.wikipedia.org/wiki/Jetty\_\(web\_server](https://en.wikipedia.org/wiki/Jetty_%28web_server)\)
...
* **GlassFish**
  * References
    * GitHub - javaee/glassfish: The Open Source Java EE Reference Implementation : [https://github.com/javaee/glassfish](https://github.com/javaee/glassfish)
...
* 平时对外服务的 Minor GC 时间约在 30ms 以内, 完全可以接受
  ...
  * 这些数据会在内存中形成超过 100 万个 HashMap 的 Entry
...
* _在这个案例中加入该参数, 问题马上得到解决_
  * 本案例来源于 ITEye HLLVM 和群组的讨论 : [http://hllvm.group.iteye.com/group/topic/28745](http://hllvm.group.iteye.com/group/topic/28745)
  * [http://support.microsoft.comy/default.aspx?scid=kb;en-us;293215](http://support.microsoft.comy/default.aspx?scid=kb;en-us;293215)
...
* 原始案例来自 "小米云技术" 公众号
  * 原文地址 : [https://juejin.im/post/5d1b1fc46fb9a07ef7108d82](https://juejin.im/post/5d1b1fc46fb9a07ef7108d82) 笔者做了一些改动
  * [https://bugs.openjdk.java.net/browse/JDK-8161147](https://bugs.openjdk.java.net/browse/JDK-8161147)
