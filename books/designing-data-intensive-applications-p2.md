...
![routing-request-to-right-node.png](../.gitbook/assets/routing-request-to-right-node.png)
...
![linearizability-example.png](../.gitbook/assets/linearizability-example.png)
...
![2pc-two-phase-commit.png](../.gitbook/assets/2pc-two-phase-commit.png)
...
* A **2PC transaction begins with** the application reading and **writing data on multiple database nodes**, _as normal._
  ...
  * The coordinator then tracks the responses from the participants :
    * **If all participants reply "yes,"** indicating they are ready to commit, then **the coordinator sends out a commit request in phase 2**, and the commit actually takes place _\( 发生/举行 \)_ .
...
* As an alternative to 2PC, an algorithm called **three-phase commit \(3PC\)** has been proposed.
  * However, 3PC **assumes a network with bounded delay and nodes with bounded response times**;
    * \(but\) in most practical systems **with unbounded network delay and process pauses**, it **cannot guarantee atomicity**.
...
* _For example, a message from a message queue can be acknowledged as processed if and only if the database transaction for processing the message was successfully committed._
  * This is **implemented by atomically committing the message acknowledgment** _**\( 自动提交消息确认 \)**_ ****_**and**_ **the database writes in a single transaction** _**\( 数据库 \(单个事务\) 的写入 \)**_ .
...
* _All of the consensus protocols discussed so far_ **internally use a leader in some form or another**, _but they don't guarantee that the leader is unique._
  * _Instead, they can make a weaker guarantee :_ **the protocols define an epoch number** _\( 世代编号 \)_
...
* ZooKeeper is modeled after _\( 模仿/仿造 \)_ Google's Chubby lock service, implementing not only total order broadcast \( and hence consensus \) , but also _an interesting set of_ other features _that turn out to be particularly useful when building distributed systems :_
  * **Linearizable atomic operations**
    ...
    * **A distributed lock is usually implemented as a lease** _\( 租约 \)_ , which has an expiry time so that it is eventually released in case the client fails.
