...
- 取中断类型码n（2）标志寄存器入栈，**IF=0**，TF=0；
 
- CS、IP入栈（4）(ip) = (n * 4), (cs) = (n * 4 + 2)
...
- **按下**一个**键**，开关触发，芯片**产生**一个**扫描码**，
 
扫描码说明按下的键在键盘上的位置。
 
**扫描码**被送入主板上的**相关**接口芯片的寄存器中，
...
如movsb、movsw、**cmps、scas、lods、stos**等。
 
若要使用它们方便进行批量数据处理，
 
则需要**与rep、repe、repne**等**前缀指令配合**使用。
...
<div>***.完整****键盘扫描码****表：**<a href="evernote:///view/7264256/s33/57b177c1-a9e9-4727-a748-bd69042cc8e1/57b177c1-a9e9-4727-a748-bd69042cc8e1/" style="color: rgb(105, 170, 53);">键盘扫描码</a>。</div> 
**部分**如下：
